...
cursor = cnxn.cursor()
r=requests.get('https://www.lematinal.com/')
...
for x in range(len(titles)):
    cursor.execute("insert into nc_news(tTitle,tDescription,tPhoto,country_Id,category_Id,createdDate,aURL) values(?,?,?,?,?,?,?)",titles[x],descr[x],images[x],33,1,today,url[x])
