...
try:
    cursor = cnxn.cursor()
    r=requests.get('http://www.aujourdhui-en-guinee.com/')
    ...
    for x in range(len(titles)):
        cursor.execute("insert into nc_news(tTitle,tDescription,tPhoto,country_Id,category_Id,createdDate,aURL) values(?,?,?,?,?,?,?)",titles[x],descr[x],images[x],21,1,today,url[x])
