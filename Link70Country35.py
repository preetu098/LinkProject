...
cursor = cnxn.cursor()
r=requests.get('http://www.jornalsavana.com/')
...
images=[]
url=[]
for x in soup.findAll("h2", class_= "title"):
    title=x.get_text()
    myurl=x.find('a')['href']
...
for x in soup.findAll("div",class_="post-content"):
    
    descr.append(x.get_text().lstrip())
  
for x in soup.find_all('div',class_="featured-thumbnail"):
    t=x.find('img')['src']
    images.append(t)
